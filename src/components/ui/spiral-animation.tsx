...
/**
 * Spiral particle animation (canvas + gsap).
 *
 * Adapted from the original community component to match the ReputeX theme:
 * the frame clears to the site's dark-navy background and particles are drawn
 * in neon cyan instead of white-on-black, so it sits naturally behind the dark
 * UI. Star count is also reduced since it runs as a passive background.
 */

// Theme colors (kept in sync with src/index.css --background / neon palette).
const CLEAR_COLOR = 'hsl(220, 70%, 5%)'
const PARTICLE_COLOR = '#22e0ff'
const TRAIL_COLOR = '#3ec8ff'
...
class AnimationController {
    ...
    // 常量 — cameraZ / viewZoom are public so Star can project against them.
    private readonly changeEventTime = 0.32
    public readonly cameraZ = -400
    private readonly cameraTravelDistance = 3400
    private readonly startDotYOffset = 28
    public readonly viewZoom = 100
    private readonly numberOfStars = 2600
    ...
    constructor(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D, dpr: number, size: number) {
        ...
        // 初始化 — seeded star creation happens inside setupRandomGenerator().
    ...
    private drawTrail(t1: number) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            this.ctx.fillStyle = TRAIL_COLOR
            this.ctx.lineWidth = sw
